...
/**
 * Checks if the user is the ownser of the quiz; if not, emits a 403 error.
 */
function mustOwnQuiz($quizID){
    global $dbh;
    if(key_exists('user-id', $_SESSION)){
        try {
            $statement = $dbh->prepare('select authorId from Quizzes where id = :quizId');
            $statement->execute([
                ':quizId' => $quizID,
            ]);
            $row = $statement->fetch(PDO::FETCH_ASSOC);
            if($_SESSION['user-id'] != $row["authorId"]){
                error("You must be the owner to perform that action.", 403);
            }
        } catch(PDOException $e){
            error("Error fetching quiz", 500);
        }
    }
    else{
        error("You must be signed in to perform that action.", 403);
...
function getQuizIdFromQuizItem($id){
    try {
        $statement = $dbh->prepare('select quizId from QuizItems where id = :id');
        $statement->execute([
            ':id' => $id]);
        ...
        error("There was an error fetching the quiz", 500);
    }
}
...
function removeQuizItem($data){
    ...
    // authenticate($data['username'], $data['password']);
    mustBeSignedIn();
    $quizId = getQuizIdFromQuizItem($data["quizItemId"]);
    mustOwnQuiz($quizId);
